...
def findtargets (mrna,refpath,ie,outfiles,plp_length=30,gc_min=50,gc_max=65,ligationsite_GC=True):
    ...
    for i in range(0, end):
        ...
        if central_condition:
            gc_frac = gc_fraction(mrna.seq[i:i+plp_length])*100
            if gc_frac > gc_min and gc_frac < gc_max:
                triple_A_count = mrna.seq[i:i+plp_length].count("AAA")
                triple_C_count = mrna.seq[i:i+plp_length].count("CCC")
                triple_G_count = mrna.seq[i:i+plp_length].count("GGG")
                triple_T_count = mrna.seq[i:i+plp_length].count("TTT")
                if triple_A_count + triple_C_count + triple_G_count + triple_T_count == 0:
                    new_target = {'Gene': mrna.id, 'Position': i, 'Sequence': str(mrna.seq[i:i+plp_length])}
                    targets = pd.concat([targets, pd.DataFrame([new_target])], ignore_index=True)
...
def select_sequences(path,selected,genes_required,number_of_selected,subgroup=1):
    ...
    for e in unigene:
        ele=pan[pan['Gene']==e]
        print(f'We are testing {number_of_selected} out of {ele.shape[0]} probes in gene _{e}_')
        if ele.shape[0]<number_of_selected:
            selec=ele
        else:    
            for num in range(0,number_of_selected):
                # if ele.shape[0]>0: The previous condition fulfills this criteria (ele.shape[0] > number_of_selected to continue here)
                    ...
                    try:
                        ...
                        seleall=sele
                    exclude=list(range(int(sele['Position'].iloc[0]-20),int(sele['Position'].iloc[0]+20)))
